...
/*
 * Archivo: GuiLauncher.cpp
 * Papel técnico: punto de salto pequeño hacia el runtime visual cuando la compilación incluye ImGui.
 ...
 */
/*
 * TLALPOWA_COMENTARIO_TECNICO_V3 · fusión mensual sin regresión · GuiLauncher.cpp:14 · bloque 1
...
/*
 * Función run_gui_launcher: orquesta una operación completa con límites claros de inicialización, progreso, cancelación, efectos laterales y recuperación ante error.
 * Contexto: pertenece a punto de salto pequeño hacia el runtime visual cuando la compilación incluye ImGui. Las entradas deben interpretarse según contratos ya normalizados por el módulo llamador, y cualquier tolerancia a datos sucios debe permanecer explícita para que un refactor no confunda ausencia de datos con cero, falso o selección vacía.
 ...
 * Para la compuerta de activación gráfica, que evita cargar dependencias de UI cuando la compilación o el modo de ejecución no lo requieren, esta separación evita que un ajuste de interfaz termine alterando silenciosamente la semántica analítica.
 */
 ...
 * Dentro de la compuerta de activación gráfica, que evita cargar dependencias de UI cuando la compilación o el modo de ejecución no lo requieren, esta disciplina evita que una capa posterior compense silenciosamente una falla anterior.
 */
